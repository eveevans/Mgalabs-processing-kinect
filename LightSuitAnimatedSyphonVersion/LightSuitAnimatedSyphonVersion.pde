...
void setup() {  
  ...
  server = new SyphonServer(this, "trajeLuz");
...
void draw() {
  ...
  if( (sk_right_hand.y > 0 && sk_right_hand.y <= 160) || ( sk_left_hand.y > 0 && sk_left_hand.y <= 160 )  ){
     ...
     draw_rotated_body( suits[2], sk_left_shoulder, sk_right_shoulder, sk_torso);
   }
     ...
     draw_rotated_body( suits[1], sk_left_shoulder, sk_right_shoulder, sk_torso);
   }
   else {
...
void draw_rotated_image(PImage image_to_draw, PVector a, PVector b){
  ...
  canvas.rotate(angle);
  canvas.image(image_to_draw, 0, 0);
  canvas.rotate(-angle);
  canvas.translate(-a.x,-a.y);
}

void draw_rotated_head(PImage image_to_draw, PVector a, PVector b){
  vtemp = PVector.sub(b,a);
  angle = atan2(vtemp.y, vtemp.x);
  canvas.translate(a.x, a.y);
  canvas.rotate(angle);
...
void draw_rotated_body(PImage image_to_draw, PVector a, PVector b, PVector center){
  ...
  canvas.rotate(angle);
  canvas.image(image_to_draw, -image_to_draw.width/2,-image_to_draw.height/2);
  //rotate(-angle);
...
void oscEvent(OscMessage message) {
  ...
  if(message.checkAddrPattern("/Head")==true) sk_head.set(message.get(0).floatValue()*width, message.get(1).floatValue()*height-20, message.get(2).floatValue());
